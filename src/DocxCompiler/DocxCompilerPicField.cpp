...
p_placeHolderNode) {
  ...
  FOR_EACH(l_valueByParamIterator, p_valueByParam) {
    ...
    switch (l_param) {
      case PARAM_USE_IMAGE_FILE_SIZE: m_useSize = USE_IMAGE_FILE_SIZE;
        break;
      case PARAM_USE_PLACE_HOLDER_WIDTH: m_useSize = USE_PLACE_HOLDER_WIDTH;
        break;
      case PARAM_USE_PLACE_HOLDER_HEIGHT: m_useSize = USE_PLACE_HOLDER_HEIGHT;
        break;
      case PARAM_USE_PLACE_HOLDER_SIZE: m_useSize = USE_PLACE_HOLDER_SIZE;
        break;
    }
  }
} // c'tor
...
void DocxCompilerPicField::serialize(ZipFile* p_zipFile) {
  ...
  p_zipFile ->writeNum<uint64_t>(m_picId);
  p_zipFile ->writeNum<uint64_t>(m_emuWidth);
  p_zipFile ->writeNum<uint64_t>(m_emuHeight);
  p_zipFile ->writeNum<int16> (m_useSize);

  p_zipFile ->writeNum<uint64_t>((uint64_t) m_picStrings.size());
