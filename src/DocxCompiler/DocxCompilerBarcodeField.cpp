...
p_placeHolderNode) {
  ...
  FOR_EACH(l_valueByParamIterator, p_valueByParam) {
    ...
    switch (l_param) {
      case PARAM_CODE11: m_barcodeFormat ->m_symbology = BARCODE_CODE11;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CODE25: m_barcodeFormat ->m_symbology = BARCODE_C25MATRIX;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CODE25_IATA: m_barcodeFormat ->m_symbology = BARCODE_C25IATA;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CODE25_INTER: m_barcodeFormat ->m_symbology = BARCODE_C25INTER;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CODE25_LOGIC: m_barcodeFormat ->m_symbology = BARCODE_C25LOGIC;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CODE25_IND: m_barcodeFormat ->m_symbology = BARCODE_C25IND;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_ITF14: m_barcodeFormat ->m_symbology = BARCODE_ITF14;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CODE39: m_barcodeFormat ->m_symbology = BARCODE_CODE39;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CODE39_EXT: m_barcodeFormat ->m_symbology = BARCODE_EXCODE39;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CODE93: m_barcodeFormat ->m_symbology = BARCODE_CODE93;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_PZN: m_barcodeFormat ->m_symbology = BARCODE_PZN;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_LOGMARS: m_barcodeFormat ->m_symbology = BARCODE_LOGMARS;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CODE32: m_barcodeFormat ->m_symbology = BARCODE_CODE32;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CODE128: m_barcodeFormat ->m_symbology = BARCODE_CODE128;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CODE128B: m_barcodeFormat ->m_symbology = BARCODE_CODE128B;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_EAN128: m_barcodeFormat ->m_symbology = BARCODE_EAN128;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_EAN14: m_barcodeFormat ->m_symbology = BARCODE_EAN14;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_NVE18: m_barcodeFormat ->m_symbology = BARCODE_NVE18;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_UPCA: m_barcodeFormat ->m_symbology = BARCODE_UPCA;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_UPCE: m_barcodeFormat ->m_symbology = BARCODE_UPCE;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_EAN: m_barcodeFormat ->m_symbology = BARCODE_EANX;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_ISBN: m_barcodeFormat ->m_symbology = BARCODE_ISBNX;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CODABAR: m_barcodeFormat ->m_symbology = BARCODE_CODABAR;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_PHARMA: m_barcodeFormat ->m_symbology = BARCODE_PHARMA;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_PHARMA_2TRACK: m_barcodeFormat ->m_symbology = BARCODE_PHARMA_TWO;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_PLESSEY: m_barcodeFormat ->m_symbology = BARCODE_PLESSEY;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_MSI_PLESSEY: m_barcodeFormat ->m_symbology = BARCODE_MSI_PLESSEY;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_TELEPEN: m_barcodeFormat ->m_symbology = BARCODE_TELEPEN;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_TELEPEN_NUM: m_barcodeFormat ->m_symbology = BARCODE_TELEPEN_NUM;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_RSS14: m_barcodeFormat ->m_symbology = BARCODE_RSS14;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_RSS14_STACK: m_barcodeFormat ->m_symbology = BARCODE_RSS14STACK;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_RSS14_STACKOMNI: m_barcodeFormat ->m_symbology = BARCODE_RSS14STACK_OMNI;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_RSS_LTD: m_barcodeFormat ->m_symbology = BARCODE_RSS_LTD;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_RSS_EXP: m_barcodeFormat ->m_symbology = BARCODE_RSS_EXP;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_RSS_EXPSTACK: m_barcodeFormat ->m_symbology = BARCODE_RSS_EXPSTACK;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CHANNEL: m_barcodeFormat ->m_symbology = BARCODE_CHANNEL;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_FIM: m_barcodeFormat ->m_symbology = BARCODE_FIM;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_FLAT: m_barcodeFormat ->m_symbology = BARCODE_FLAT;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_DAFT: m_barcodeFormat ->m_symbology = BARCODE_DAFT;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_POSTNET: m_barcodeFormat ->m_symbology = BARCODE_POSTNET;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_PLANET: m_barcodeFormat ->m_symbology = BARCODE_PLANET;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_ONECODE: m_barcodeFormat ->m_symbology = BARCODE_ONECODE;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_RM4SCC: m_barcodeFormat ->m_symbology = BARCODE_RM4SCC;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_DPLEIT: m_barcodeFormat ->m_symbology = BARCODE_DPLEIT;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_DPIDENT: m_barcodeFormat ->m_symbology = BARCODE_DPIDENT;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_AUSPOST: m_barcodeFormat ->m_symbology = BARCODE_AUSPOST;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_AUSREPLY: m_barcodeFormat ->m_symbology = BARCODE_AUSREPLY;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_AUSROUTE: m_barcodeFormat ->m_symbology = BARCODE_AUSROUTE;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_AUSREDIRECT: m_barcodeFormat ->m_symbology = BARCODE_AUSREDIRECT;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_KIX: m_barcodeFormat ->m_symbology = BARCODE_KIX;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_JAPANPOST: m_barcodeFormat ->m_symbology = BARCODE_JAPANPOST;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_KOREAPOST: m_barcodeFormat ->m_symbology = BARCODE_KOREAPOST;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CODE16K: m_barcodeFormat ->m_symbology = BARCODE_CODE16K;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CODE49: m_barcodeFormat ->m_symbology = BARCODE_CODE49;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_PDF417: m_barcodeFormat ->m_symbology = BARCODE_PDF417;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_PDF417_TRUNC: m_barcodeFormat ->m_symbology = BARCODE_PDF417TRUNC;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_MICROPDF: m_barcodeFormat ->m_symbology = BARCODE_MICROPDF417;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_DATAMATRIX: m_barcodeFormat ->m_symbology = BARCODE_DATAMATRIX;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_QRCODE: m_barcodeFormat ->m_symbology = BARCODE_QRCODE;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_MICROQR: m_barcodeFormat ->m_symbology = BARCODE_MICROQR;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_MAXICODE: m_barcodeFormat ->m_symbology = BARCODE_MAXICODE;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_AZTEC: m_barcodeFormat ->m_symbology = BARCODE_AZTEC;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_AZRUNE: m_barcodeFormat ->m_symbology = BARCODE_AZRUNE;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CODEONE: m_barcodeFormat ->m_symbology = BARCODE_CODEONE;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_GRIDMATRIX: m_barcodeFormat ->m_symbology = BARCODE_GRIDMATRIX;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_HIBC_CODE39: m_barcodeFormat ->m_symbology = BARCODE_HIBC_39;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_HIBC_CODE128: m_barcodeFormat ->m_symbology = BARCODE_HIBC_128;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_HIBC_PDF417: m_barcodeFormat ->m_symbology = BARCODE_HIBC_PDF;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_HIBC_MICROPDF: m_barcodeFormat ->m_symbology = BARCODE_HIBC_MICPDF;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_HIBC_DATAMATRIX: m_barcodeFormat ->m_symbology = BARCODE_HIBC_DM;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_HIBC_QRCODE: m_barcodeFormat ->m_symbology = BARCODE_HIBC_QR;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_HIBC_AZTEC: m_barcodeFormat ->m_symbology = BARCODE_HIBC_AZTEC;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CS_EAN: m_barcodeFormat ->m_symbology = BARCODE_EANX_CC;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CS_EAN128: m_barcodeFormat ->m_symbology = BARCODE_EAN128_CC;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CS_UPCA: m_barcodeFormat ->m_symbology = BARCODE_UPCA_CC;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CS_UPCE: m_barcodeFormat ->m_symbology = BARCODE_UPCE_CC;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CS_RSS14: m_barcodeFormat ->m_symbology = BARCODE_RSS14_CC;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CS_RSS14_STACK: m_barcodeFormat ->m_symbology = BARCODE_RSS14STACK_CC;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CS_RSS14_STACKOMNI: m_barcodeFormat ->m_symbology = BARCODE_RSS14_OMNI_CC;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CS_RSS_LTD: m_barcodeFormat ->m_symbology = BARCODE_RSS_LTD_CC;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CS_RSS_EXP: m_barcodeFormat ->m_symbology = BARCODE_RSS_EXP_CC;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
      case PARAM_CS_RSS_EXPSTACK: m_barcodeFormat ->m_symbology = BARCODE_RSS_EXPSTACK_CC;
        m_barcodeFormat ->m_symbologyName = DocxCompilerField::paramToStr(l_param);
        break;
  ...
  if (l_sz == 0 || l_szCs == 0 || l_fgcolor == "") {
    ...
    while (l_cursorNode = (xercesc::DOMElement*) l_treeDriller ->nextNode()) {
      ...
      if (l_nodeName == "sz") {
        if (l_sz == 0) l_sz = StrFunc::strToInt<unsigned short>(l_attrValue);
      } else if (l_nodeName == "szCs") {
        if (l_szCs == 0) l_szCs = StrFunc::strToInt<unsigned short>(l_attrValue);
      } else if (l_nodeName == "color") {
        if (l_fgcolor.empty()) l_fgcolor = StrFunc::normalizeColor(l_attrValue);
      } else if (l_nodeName == "highlight") {
        if (l_bgcolor.empty()) l_bgcolor = StrFunc::normalizeColor(l_attrValue);
      }
    }
  }
