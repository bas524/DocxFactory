...
string WinFunc::getSubPath(
  ...
  for (l_pos = p_from; l_pos <= p_to; ++l_pos) {
    switch (SubPathPos(l_pos)) {
      case SUBPATH_PROTOCOL: l_retVal += l_protocol;
        break;
      case SUBPATH_HOST: l_retVal += l_host;
        break;
      case SUBPATH_SHARED_DIR: l_retVal += l_sharedDir;
        break;
      case SUBPATH_DRIVE: l_retVal += l_drive;
        break;
