...
uint64_t DocxMergerChartAxisNumber::setValue(DocxMergerPasteChartAxis* p_pasteAxis, const string& p_value) {
  double l_value = LocaleFunc::strToNum(p_value);

  return ( (DocxMergerPasteChartAxisNumber*) p_pasteAxis) ->setValue(l_value);
} // setValue

uint64_t DocxMergerChartAxisNumber::setValue(DocxMergerPasteChartAxis* p_pasteAxis, double p_value) {
  return ( (DocxMergerPasteChartAxisNumber*) p_pasteAxis) ->setValue(p_value);
} // setValue

string DocxMergerChartAxisNumber::getValue(DocxMergerPasteChartAxis* p_pasteAxis, uint64_t p_index) const {
  return StrFunc::intToStr(((DocxMergerPasteChartAxisNumber*) p_pasteAxis) ->getValues() ->at(p_index), 3);
} // getValue

uint64_t DocxMergerChartAxisNumber::getLength(DocxMergerPasteChartAxis* p_pasteAxis) const {
