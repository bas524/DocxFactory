...
void DocxMergerChartFieldSeries::setPasteChartValue(Series_T p_series, Category_T p_category, double p_value) {
  ...
  uint64_t l_series = m_serAxis ->setValue((DocxMergerPasteChartAxis*) l_pasteField ->getPasteSerAxis(), p_series);
  uint64_t l_category = m_catAxis ->setValue((DocxMergerPasteChartAxis*) l_pasteField ->getPasteCatAxis(), p_category);
...
void DocxMergerChartFieldSeries::deserialize(UnzipFile* p_unzipFile) {
  ...
  switch (m_catType) {
    case DocxMergerField::TYPE_TEXT: m_catAxis = new DocxMergerChartAxisText();
      break;
    case DocxMergerField::TYPE_NUMBER: m_catAxis = new DocxMergerChartAxisNumber();
      break;
    case DocxMergerField::TYPE_DATE_TIME: m_catAxis = new DocxMergerChartAxisDateTime();
      break;
  }
} // deserialize
