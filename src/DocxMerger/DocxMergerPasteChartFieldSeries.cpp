...
DocxMergerPasteChartFieldSeries::DocxMergerPasteChartFieldSeries(DocxMergerField* p_field) : DocxMergerPasteChartField(p_field) {
  switch (((DocxMergerChartFieldSeries*) p_field) ->getCatType()) {
    case DocxMergerField::TYPE_TEXT: m_pasteCatAxis = new DocxMergerPasteChartAxisText();
      break;
    case DocxMergerField::TYPE_NUMBER: m_pasteCatAxis = new DocxMergerPasteChartAxisNumber();
      break;
    case DocxMergerField::TYPE_DATE_TIME: m_pasteCatAxis = new DocxMergerPasteChartAxisDateTime();
      break;
  }
...
string* DocxMergerPasteChartFieldSeries::getValue() {
  ...
  uint64_t l_strLen = l_seriesStrings ->size();
  uint64_t l_serLen = l_serAxis ->getLength(m_pasteSerAxis);
